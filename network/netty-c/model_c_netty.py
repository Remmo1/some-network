...
# Predykcja dla Netty 4.1.120 z datą 2025-05-01
new_sample = pd.DataFrame([{
    'year': 2025, 'month': 5, 'day': 1,
    'ver_major': 4, 'ver_minor': 1, 'ver_patch': 120
}])
new_sample_scaled = scaler.transform(new_sample)
prediction = model.predict(new_sample_scaled)
pred_class = np.argmax(prediction)
print(f"Prognozowana liczba CVE (mediana) dla Netty 4.1.120: {le.inverse_transform([pred_class])[0]}")
